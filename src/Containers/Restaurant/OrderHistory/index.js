  ...
  BackHandler.addEventListener("hardwareBackPress", this.handleBackPress);
    ...
    await AsyncStorage.getItem('userLoginInfo').then(async (user) => {
      ...
      this.focusListener = navigation.addListener("didFocus", () => {
        ...
        this.getOrderHistory(userData.id)
      });
      this.getOrderHistory(userData.id)
    })
  }
...
var name = ""
console.log(rowData)
rowData.data && rowData.data.map((item)=>{
  name = name + item.FoodName + " (" + item.count + "),"
})
    return (
      <View style={{...styles.mainRenderView}}>
        
         <TouchableOpacity 
                onPress={()=>this.props.navigation.navigate("OrderDetailsRes", {item:rowData})}
                activeOpacity={0.5}
                >
        <View style={{ flexDirection: "row" }}>
          <Image source={{uri:rowData.data[0].FoodImg? rowData.data[0].FoodImg:this.state.dummy}} style={{...styles.FoodImg}} />
          <View>
            <Text style={styles.FoodName}>{name}</Text>
            <Text style={styles.FoodAdd}>Payment: {rowData.payment == "offline"? "Cash on Delivery": "Done"}</Text>
           </View>
        </View>
